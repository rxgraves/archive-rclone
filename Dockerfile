...
# Install curl + unzip + tzdata + sudo (for date command)
RUN apt-get update && \
    apt-get install -y curl unzip ca-certificates tzdata sudo && \
    ln -fs /usr/share/zoneinfo/UTC /etc/localtime && \
    dpkg-reconfigure -f noninteractive tzdata && \
    echo "appuser ALL=(ALL) NOPASSWD: /bin/date" >> /etc/sudoers && \
    apt-get clean && \
...
# FINAL RELIABLE TIME SYNC (with sudo)
ENTRYPOINT ["/bin/bash", "-c", \
    "echo 'Forcing time sync via HTTP...' && \
     ...
     if [ -n \"$TIME\" ]; then \
       ...
       sudo date -u -s \"$CLEAN_TIME\" > /dev/null 2>&1 && \
